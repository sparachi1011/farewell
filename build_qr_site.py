...
# The builder calls your methods from alvida.py so the webpage text always matches your script. [1](https://ykgwoffice-my.sharepoint.com/personal/parachi_saikoushik_yokogawa_com/Documents/Microsoft%20Copilot%20Chat%20Files/alvida.py)
import alvida

# Mirror your colleagues list from alvida.py's __main__ block. [1](https://ykgwoffice-my.sharepoint.com/personal/parachi_saikoushik_yokogawa_com/Documents/Microsoft%20Copilot%20Chat%20Files/alvida.py)
COLLEAGUES = [
    ['Shiozaki san', 'Shrikanth san', 'Furuya san'],
    ['Rajeev san'], ['Jeeth'], ['Sumitra'],
    ['Pankaj', 'Gouse', 'Neel', 'Karthik', 'Chaithanya', 'Sudeep'],
    ['Deepali', 'Shreyanshi', 'Jyoti', 'Akshata', 'Shiv', 'Ajay', 'Sandhya', 'Amar'],
...
def build_messages():
    ...
    from your alvida.py so the webpage text always matches your Python script. [1](https://ykgwoffice-my.sharepoint.com/personal/parachi_saikoushik_yokogawa_com/Documents/Microsoft%20Copilot%20Chat%20Files/alvida.py)
    """
    farewell = alvida.Alvida.say_farewell()
    closing = alvida.Alvida.say_closing_comments()

    # Per-person messages via your class to keep content identical. [1](https://ykgwoffice-my.sharepoint.com/personal/parachi_saikoushik_yokogawa_com/Documents/Microsoft%20Copilot%20Chat%20Files/alvida.py)
    per_person = {}
    for group in COLLEAGUES:
        for name in group:
            per_person[name] = alvida.Alvida(name).say_goodbye()

    # Default (else-branch) fallback from your code. [1](https://ykgwoffice-my.sharepoint.com/personal/parachi_saikoushik_yokogawa_com/Documents/Microsoft%20Copilot%20Chat%20Files/alvida.py)
    default_msg = alvida.Alvida("Colleague").say_goodbye()
    return farewell, per_person, closing, default_msg


# ------------------------------
# Token index for client-side matcher (mirrors your normalize logic)
# ------------------------------
...
def load_translations(path: str | None) -> dict:
    ...
    {
      "ui": {
        "en": {"title": "Farewell — Python Console",
               "placeholder": "Please enter your Name/Email to see Farewell Message",
               "direct_link": "Direct link"},
        "hi": {"title": "फेयरवेल — पायथन कंसोल",
               "placeholder": "फेयरवेल संदेश देखने के लिए अपना नाम/ईमेल दर्ज करें",
               "direct_link": "सीधा लिंक"},
        "ja": {"title": "送別 — Python コンソール",
               "placeholder": "送別メッセージを見るには氏名/メールを入力してください",
               "direct_link": "直接リンク"}
      },
      "content": {
        "hi": {
          "farewell": "...(Hindi translation of farewell)...",
          "closing":  "...(Hindi translation of closing)...",
          "default":  "...(Hindi translation of default fallback)...",
          "messages": {
            "Shiv": "...",
            "Rajeev san": "...",
            "...": "..."
    ...
    p = Path(path)
    if not p.exists():
        print(f"[!] Translations file not found at {path}. Continuing without it.")
        return {}
    try:
...
def render_html(final_url: str,
    ...
    Write a Yokogawa-themed, Python-console-styled single HTML file, with:
      ...
      - your default fallback when no match
    """
...
<style>
  ...
  /* Light theme variables override when body[data-theme="light"] is set */
  ...
  * {{ box-sizing: border-box; }}
  ...
  /* Header with logo + title + controls (lang & theme) */
  .header {{
    display: grid;
  ...
  /* <pre> text wraps responsively on any screen */
  .terminal pre {{
    margin: 0;
  ...
  /* Typewriter caret for the typed blocks */
  .typing::after {{
...
<body>
  <div class="wrap">
    ...
    <div class="header">
      {logo_img_tag}
      <h1 id="title" class="title">Farewell — Python Console</h1>
      <div class="controls">
        <!-- Language selector -->
    ...
    <!-- INPUT (BEFORE the first console, grey background, updated placeholder) -->
    <div class="inputWrap">
      <input id="who" type="text"
        placeholder="Please enter your Name/Email to see Farewell Message" />
    </div>

    <!-- FIRST CONSOLE: heading & farewell -->
    <div class="terminal" style="margin-top:8px;">
<span class="prompt">&gt;&gt;&gt;</span> from alvida import Alvida
...
<span class="prompt">&gt;&gt;&gt;</span> Alvida.say_closing_comments()
...
<script>
  // -----------------------------
  // Data injected from Python (sourced from alvida.py)  [1](https://ykgwoffice-my.sharepoint.com/personal/parachi_saikoushik_yokogawa_com/Documents/Microsoft%20Copilot%20Chat%20Files/alvida.py)
  // -----------------------------
  ...
  const defaultMsgEN   = {default_json}; // from Alvida("Colleague").say_goodbye()  [1](https://ykgwoffice-my.sharepoint.com/personal/parachi_saikoushik_yokogawa_com/Documents/Microsoft%20Copilot%20Chat%20Files/alvida.py)

  // Optional translations payload (UI + content)
  // See Python docstring for format.
  const TRANSLATIONS = {translations_json};

  const MIN = 4;

  // -----------------------------
  // UI strings by language (fallbacks if TRANSLATIONS.ui is missing)
  // -----------------------------
  const UI_FALLBACK = {{
    en: {{
      title: "Farewell — Python Console",
      placeholder: "Please enter your Name/Email to see Farewell Message",
      direct_link: "Direct link"
    }},
    hi: {{
  ...
  // -----------------------------
  // Normalization (mirrors Python logic)  [1](https://ykgwoffice-my.sharepoint.com/personal/parachi_saikoushik_yokogawa_com/Documents/Microsoft%20Copilot%20Chat%20Files/alvida.py)
  // -----------------------------
  ...
  function initTheme() {{
    ...
    // React to OS changes only when on 'auto'
  ...
  function applyLanguage(lang) {{
    ...
    // Re-type farewell & closing in selected language (or fallback)
  ...
  // -----------------------------
  // Result console (uses translations if provided)
  // -----------------------------
  ...
  function showMessageForInput(rawInput) {{
    ...
    if (name) {{
      // Prefer translated message if provided; else English; else default
      ...
      // Fallback: your default message (translated if available)
...
# ------------------------------
def main():
    parser = argparse.ArgumentParser(description="Build Yokogawa-themed Farewell site + QR (uses alvida.py)")
    parser.add_argument("--url", required=True, help="Public URL where index.html will be hosted (e.g., GitHub Pages link).")
    parser.add_argument("--out", default="site", help="Output folder (default: site)")
    parser.add_argument("--logo", default="Yokogawa_logo.png", help="Path to the logo image to place at top-left.")
    parser.add_argument("--translations", default=None, help="Optional JSON with UI/content translations.")
    args = parser.parse_args()

    out_dir = Path(args.out)
    out_dir.mkdir(parents=True, exist_ok=True)

    # Gather English baseline content from your alvida.py (keeps page in sync with your script). [1](https://ykgwoffice-my.sharepoint.com/personal/parachi_saikoushik_yokogawa_com/Documents/Microsoft%20Copilot%20Chat%20Files/alvida.py)
